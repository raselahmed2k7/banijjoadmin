...
class FeturedCategory extends Component {
  constructor(props) {
    super(props);
  ...
  toggleSmall(event) {
    ...
    if (event == 'deleteCategoryOrderListPermitted') {
      fetch(base+'/api/category_feature_list_delete', {
        method: 'GET'
      })
      .then(res => {
        console.log(res);
      ...
      .then(info => {
        ...
        if (info.success == true) {
          ToastsStore.success("Feature Category Successfully deleted !!");
          ...
          ToastsStore.warning("Feature Category delete Faild. Please try again !!");
          console.log(info.success);
        }
  ...
  addClick(){
    ...
    if (this.state.PurchaseList.length < 1) {
  ...
  limitExitMessage () {
    this.state.limitExitMessageString = 'Limitation crossed ! Maximum size is 1 !';
  }
  ...
  getCategoryOrderList () {
    fetch(base+'/api/category_feature_list', {
      method: 'GET'
    })
    .then(res => {
      console.log(res);
  ...
  handleSubmit(event) {
    ...
    if (this.state.PurchaseList.length == 1) {
      fetch(base+'/api/save_feature_category' , {
        method: "POST",
        headers: {
          'Content-type': 'application/json'
        },
        body: JSON.stringify(this.state.PurchaseList)
      })
      ...
      .then((info) => { 
        ...
        if (info.success == true) {
          ToastsStore.success("Feature Category Successfully inserted !!");
          console.log(info.success);
          this.state.PurchaseList = [];
          this.setState({
            limitExitMessageString: ''
          ...
          ToastsStore.warning("Feature Category Insertion Faild. Please try again !!");
          console.log(info.success);
        }
      ...
      ToastsStore.warning('Limitation Issue !! The size is : '+this.state.PurchaseList.length+' Need At Least 1');
  ...
  render() {
    ...
    return (
      ...
      <Col xs="12" md="6">
        <Card>
          <CardHeader>
            <strong>Add Feature Category </strong> 
          </CardHeader>
      ...
      <Col xs="12" lg="6">
        <Card>
          ...
          <CardHeader>
            <Row>
              <Col md="6">
                <i className="fa fa-align-justify"></i>Feature Category List
              </Col>
              <Col md="6">
              </Col>
            </Row>
      ...
      <ModalBody>
        <strong> 
        <center>
          Are Sure to delete this feature category list ?
        </center>
        </strong>
      </ModalBody>
      <ModalFooter>
